...
const TimeLineSlider = L.Control.extend({
    ...
    onAdd: function(map) {
        ...
        this.container = L.DomUtil.create('div', 'control_container_'+this.uuid);

        /* Prevent click events propagation to map */
        L.DomEvent.disableClickPropagation(this.container);

        /* Prevent right click event propagation to map */
        L.DomEvent.on(this.container, 'control_container_'+this.uuid, function (ev)
        {
        ...
        L.DomEvent.on(this.rangeInput, "input", function() {
            ...
            let allChangeMapParameters = {...mapParams, ...that.options.extraChangeMapParams};
        ...
        for (var li of this.rangeLabelArray) {
    ...
    getTrackStyle: function (el, sliderLength) {  
        ...
        for (var i = 0; i < prefs.length; i++) {
            style += `.range_${this.uuid} {background: linear-gradient(to right, ${that.coverBackgroundRGBA} 0%, ${that.coverBackgroundRGBA} ${coverVal}%, ${that.options.activeColor} ${coverVal}%, ${that.options.activeColor} ${val}%,  ${that.coverBackgroundRGBA} 0%, ${that.coverBackgroundRGBA} 100%)}`;
            style += `.range_${this.uuid} input::-` + prefs[i] + `{background: linear-gradient(to right, ${that.coverBackgroundRGBA} 0%, ${that.coverBackgroundRGBA} ${coverVal}%, ${that.options.activeColor} 0%, ${that.options.activeColor} ${val}%, ${that.options.inactiveColor} ${val}%, ${that.options.inactiveColor} ${100-coverVal}%, ${that.coverBackgroundRGBA} ${100-coverVal}%, ${that.coverBackgroundRGBA} 100%)}`;
        }
...
export const timelineSlider = function(options) {
